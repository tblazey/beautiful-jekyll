...
\\[ S_i = S_0\exp(-\textit{b_i}\textbf{g_i}^T\textbf{Dg_i}) \\]

where \\( \S_i \\) is the *i<sup>th</sup>* diffusion weighted image, \\( \S_0 \\) is the signal from the image(s) without any diffusion weighting, \\( \textit{b_i} \\) is the diffusion weighting (commonly refered to as b-values), and \\( \textit{g_i} \\) is a vector containing the direction of the *i<sup>th</sup>* diffusion gradient.
